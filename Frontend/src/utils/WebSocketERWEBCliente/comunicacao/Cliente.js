...
export class ClienteWS {
    ...
    processaMensagemWebSocket(rawdata) {
        ...
        try {
            const jsonMensagem = JSON.parse(rawdata.data.toString())
        ...
        if (mensagemWebSocket.isUnica) {
            ...
            // const bufferPayloadTransmissao = atob(mensagemWebSocket.unica.textoBase64)
            let jsonTransmissao = {};
            try {
                ...
                async (respostaComando, transmissao) => {
                    ...
                    if (respostaComando.resposta.sucesso) {
                        ...
                        if (respostaComando.resposta.erro.isComandoNaoExiste) {
                            retornoComando.erro.descricao = `O comando solicitado ${comando} não existe.`;
                            retornoComando.erro.isComandoNaoExiste = true;
                        } else if (respostaComando.resposta.erro.isErroExecucaoComando) {
                            retornoComando.erro.descricao = `Erro ao executar o comando ${comando}: ${respostaComando.resposta.erro.erroExecucaoComando.descricao}`;
